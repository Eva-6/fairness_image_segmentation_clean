...
from utils.utils import (NUM_CLASS, ORGAN_NAME, ORGAN_NAME_LOW,
                         PSEUDO_LABEL_ALL, TEMPLATE, invert_transform,
...
def sigmoid_single_organ(pred_sigmoid, organ_index, args):
    B, C, W, H, D = pred_sigmoid.shape
    if args.cpu:
        sigmoid_single_organ = torch.zeros(B, 1, W, H, D)
    else:
        sigmoid_single_organ = torch.zeros(B, 1, W, H, D).cuda()
    for b in range(B):
        ...
        for item in pseudo_label_single_organ_mapping:
            src, _ = item
            sigmoid_single_organ[b][0] = pred_sigmoid[b][src - 1]
    return sigmoid_single_organ
...
def validation(model, ValLoader, val_transforms, args):
    ...
    if args.suprem:
        ...
        for index, batch in enumerate(tqdm(ValLoader)):
            ...
            # if os.path.exists(os.path.join(case_save_path, "combined_labels.nii.gz")):
            #     print(f"Results already exists, skipping")
            #     continue
            ...
            if args.store_result:
                ...
                #     pseudo_label_single = pseudo_label_single_organ(
                #         pred_hard_post, organ_index, args
                #     )
                ...
                for organ_index in organ_index_all:
                    sigmoid_single = sigmoid_single_organ(
                        pred_sigmoid, organ_index, args
                    )
                    organ_name = ORGAN_NAME_LOW[organ_index - 1]
                    batch[organ_name] = sigmoid_single.cpu()
                    BATCH = invert_transform(organ_name, batch, val_transforms)
                    organ_invertd = np.squeeze(BATCH[0][organ_name].numpy(), axis=0)
                    # save organ sigmoids as the np.float32 type
                    organ_inverted_type = organ_invertd.astype(np.float32)
                    organ_save = nib.Nifti1Image(organ_inverted_type, affine_temp)
                    new_name = os.path.join(
                        organ_seg_save_path, organ_name + "_sigmoid.nii.gz"
                    )
                    print("organ sigmoid saved in path: %s" % (new_name))
                    nib.save(organ_save, new_name)
                ...
                # pseudo_label_invertd = np.squeeze(
                #     BATCH[0]["pseudo_label"].numpy(), axis=0
                # )
                # pseudo_label_inverted_type = pseudo_label_invertd.astype(np.uint8)
                # pseudo_label_save = nib.Nifti1Image(
                #     pseudo_label_inverted_type, affine_temp
                # )
